...
export default function LojaPage() {
  ...
  useEffect(() => {
    if (store && store.categories.length > 0) {
      // Priorizar categorias principais (sem parentId) no menu inicial
      const mainCategories = store.categories.filter(c => !c.parentId)
      const firstCategory = mainCategories.length > 0 ? mainCategories[0] : store.categories[0]
  ...
  const handleCheckoutContinue = (data: { customerName: string; customerPhone: string; tableId: number }) => {
    try {
      ...
      // Pequeno delay para garantir que o checkout modal fecha antes do payment modal abrir
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-black-main">
      ...
      {store.categories.length > 0 && (
        <div className="bg-dark-gray sticky top-0 z-40">
          <div className="max-w-4xl mx-auto px-4 py-3 relative">
            ...
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-dark-gray hover:bg-gold text-gold hover:text-black-pure p-2 rounded-full shadow-lg transition-colors"
            ...
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-dark-gray hover:bg-gold text-gold hover:text-black-pure p-2 rounded-full shadow-lg transition-colors"
              ...
              {store.categories
                ...
                .map((category) => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-lg text-sm font-semibold whitespace-nowrap transition-colors ${selectedCategory === category.id
                      ? 'bg-gold text-black-pure'
                      : 'bg-dark-gray text-white hover:bg-gold-bright hover:text-black-pure border border-border'
                      }`}
                  >
                    {category.name}
                  </button>
          ...
          {/* Subcategorias (Sub-menu) */}
          {selectedCategory && store.categories.filter(c => c.parentId === selectedCategory).length > 0 && (
            <div className="bg-dark-surface border-t border-border mt-0 pb-3">
              <div className="max-w-4xl mx-auto px-4 overflow-x-auto scrollbar-hide">
                <div className="flex gap-2 py-2">
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${!selectedSubCategory
                      ? 'bg-white text-dark-bg'
                      : 'bg-dark-surface text-text-muted hover:text-white border border-border'
                      }`}
                  >
                    Tudo
                  </button>
                  {store.categories
                    ...
                    .map((sub) => (
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${selectedSubCategory === sub.id
                          ? 'bg-gold text-black-pure'
                          : 'bg-dark-surface text-text-muted hover:text-white border border-border'
                          }`}
                      >
        ...
        {currentCategory && (
          <div className="mb-6">
            <h2 className="text-3xl font-bold text-white mb-2">
              {selectedSubCategory && store.categories.find(c => c.id === selectedSubCategory)?.name || currentCategory.name}
            </h2>
            {(selectedSubCategory ? store.categories.find(c => c.id === selectedSubCategory)?.description : currentCategory.description) && (
          ...
          <div className="space-y-6">
            {availableProducts.map((product) => (
              <div
                ...
                className="bg-dark-gray border-b-2 border-border pb-6 cursor-pointer hover:shadow-lg p-4 rounded-lg transition-all"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {product.description && (
                      <p className="text-gray-light mb-2 text-sm">{product.description}</p>
                    )}
